...
class TestCollectDataToEmail extends React.Component {
    state = {
      ...
      renderTableData() {
        return this.state.showOrderList.map((data, index) => {
            return (
              <div>{data}</div>
            )
        })
      ...
      addOrder = e => {
        ...
        // keep list string to state
        this.setState({
          showOrderList: showOrder
        });
      ...
      render() {
        ...
        return (
          <div>
            <button className="btn btn-secondary" onClick={this.showModal}> ส่ง email </button>
            ...
            >
              <Form>
              { <Form.Item label="เลขคำสั่งซื้อ" hasFeedback>
